...
export default async function ClientesPage() {
  ...
  const { data: clientes } = await supabase
    .from("clientes")
    .select("id, nombre, contacto, fecha_inicio, activo")
    .order("nombre");
    ...
    <div>
      <div className="flex items-end justify-between mb-8">
        <div>
          ...
          <h1 className="font-black text-3xl tracking-tight">Miembros</h1>
        </div>
        <Link
          href="/admin/clientes/nuevo"
          className="bg-accent text-white font-bold text-sm px-4 py-2.5 rounded-xl hover:bg-accent-hover transition-colors"
        >
          + Nuevo
        </Link>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        <div className="bg-surface border border-[rgba(255,255,255,0.08)] rounded-xl p-3 text-center">
          <p className="font-bold text-2xl text-text">{activos.length}</p>
          <p className="text-[10px] uppercase tracking-widest text-muted">Activos</p>
        </div>
        <div className="bg-surface border border-[rgba(255,255,255,0.08)] rounded-xl p-3 text-center">
          <p className="font-bold text-2xl text-muted">{inactivos.length}</p>
          <p className="text-[10px] uppercase tracking-widest text-muted">Inactivos</p>
        </div>
      </div>
      ...
      <div className="bg-surface border border-[rgba(255,255,255,0.08)] rounded-2xl overflow-hidden mb-6">
        <div className="px-5 py-4 border-b border-[rgba(255,255,255,0.08)]">
          <p className="text-xs uppercase tracking-widest text-muted">Activos ({activos.length})</p>
        </div>
        {activos.length === 0 ? (
            ...
            {activos.map((c) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {c.contacto && <p className="text-xs text-muted truncate">{c.contacto}</p>}
                </div>
      ...
      {inactivos.length > 0 && (
        <div className="bg-surface border border-[rgba(255,255,255,0.08)] rounded-2xl overflow-hidden opacity-60">
          <div className="px-5 py-4 border-b border-[rgba(255,255,255,0.08)]">
            <p className="text-xs uppercase tracking-widest text-muted">Inactivos ({inactivos.length})</p>
          </div>
          <div className="divide-y divide-[rgba(255,255,255,0.05)]">
            {inactivos.map((c) => (
              <Link
              ...
              >
                <div className="w-8 h-8 rounded-full bg-surface2 border border-[rgba(255,255,255,0.08)] flex items-center justify-center font-bold text-muted text-xs flex-shrink-0">
                  {c.nombre.split(" ").map((n: string) => n[0]).join("").slice(0, 2).toUpperCase()}
                </div>
